...
// Building your Test Images
    stage('BUILD') {
      parallel {
        stage('Express Image') {
          steps {
            sh 'sudo docker build -it express-image/Dockerfile \
            -t nodeapp-dev:trunk .'
          }
        }
        stage('Test-Unit Image') {
          steps {
            sh 'sudo docker build -it test-image/Dockerfile \
            -t test-image:latest .'
          }
        }
      }
...
// Performing Software Tests
    stage('TEST') {
      parallel {
        ...
        stage('Quality Tests') {
          steps {
            sh 'docker login --username datcaotien --password caocongtu'
            sh 'docker tag nodeapp-dev:trunk datcaotien/nodeapp-dev:latest'
            sh 'docker push datcaotien/nodeapp-dev:latest'
          }
        }
      }
      post {
